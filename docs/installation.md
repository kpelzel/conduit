...
3. Install [service file](./service/conduit-server.service) in /usr/lib/systemd/system/conduit-server.service
...
1. copy over `conduit-fta` binary to each fta node. This needs to be in the location specified in the the conduit runner config created earlier
